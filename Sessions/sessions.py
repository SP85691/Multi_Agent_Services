...
@session_routes.post("/{user_id}/sessions", response_model=SessionResponse)
async def create_user_session(user_id: int, session: SessionCreate, db: Session = Depends(get_db), current_user: UserResponse = Depends(get_current_user)):
    if current_user.id != user_id and not current_user.isadmin:
        raise HTTPException(status_code=403, detail="You do not have permission to create a session for this user")
    new_session = create_session(user_id, session.name, db)
    return new_session

@session_routes.get("/{user_id}/sessions", response_model=List[SessionResponse])
async def get_user_sessions(user_id: int, db: Session = Depends(get_db), current_user: UserResponse = Depends(get_current_user)):
    if current_user.id != user_id and not current_user.isadmin:
        raise HTTPException(status_code=403, detail="You do not have permission to view sessions for this user")
